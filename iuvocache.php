...
class PageCache
{
	...
	public static function doLetsGo()
	{
		try{
	...
	public static function getCachedPage()
	{
		...
		$excludes = array();
		$excludeParams = $config->iuvocache->global->params;
	...
	public static function loadConfiguration()
	{
							...
							switch((string)$child2->type){
								...
								default:
									...
									if(!self::$cacheData['path']){
										self::$cacheData['path'] = 'var/iuvocache';
									}
									break;
							}
							break;
		...
		if(!$nodeFound){
            ...
            self::report("local.xml does not contain <iuvocache> node, here let us setup  for you...", true);
	...
	public static function createLocalConfig()
	{
        ...
        $config->addChild('iuvocache');
        
        //create global section under iuvocache
        $config->iuvocache->addChild('global');
        $connection = $config->iuvocache->global->addChild('connection');
        ...
        $config->iuvocache->global->addChild('multi_currency', '0');
        $config->iuvocache->global->addChild('request_path', "<![CDATA[]]>");
	            
        //create session section under iuvocache
        $config->iuvocache->addChild('session');
        $config->iuvocache->session->addChild('type', '<![CDATA[files]]>');

        //create cache section under iuvocache
        $config->iuvocache->addChild('cache');
        $config->iuvocache->cache->addChild('type', '<![CDATA[files]]>');
        ...
        //setup htaccess file for iuvocache.php use
        $htaccess = file_get_contents('.htaccess');
        $htaccess = str_replace('index.php', 'iuvocache.php', $htaccess);
